...
def check_input(input_param, input_db, out):
    ...
    if input_param == False:
        try:
            ...
            url = input_db[0]
            wget_command = ['wget', '-c', url, '-O', os.path.join(out, os.path.basename(url))]
            ...
            with open(os.path.join(out, os.path.basename(url)), "rb") as f:
...
def check_db(db):
    if str(db).lower() == "nt" or str(db).lower() == "nucleotide":
        ...
        print("No database specified. NCBI nr protein database will be downloaded.")
        return "nr"
...
def check_db_path(db, db_path, out):
    ...
    if db_path == False:
        ...
        try:
            ...
            # compare the calculated MD5 with the expected MD5
            if db_md5.hexdigest() == expected_md5:
                print(f"The latest version of the {db_name} database has been downloaded successfully to {out}")
                return os.path.join(out, os.path.basename(db_url))
            else:
                print("MD5 checksum does not match. File might be corrupted.")
...
def merge_gz_files(file1_path, file2_path, output_path):
    wget_command = ['cat', file1_path, file2_path ,  '>',  output_path]
    try:
...
def check_acc2taxid(acc2taxid, out):
    print("\nChecking the accession to taxonomy ID mapping files")
    ...
    # If the acc2taxid is provided, then print the path and continue to the next step.
        ...
        print(f"{db_name} acc2taxid database is provided in {acc2taxid}")
        return acc2taxid
...
def filter_fasta_by_acc2taxid(fasta_path, filtered_acc2taxid_path, filtered_fasta_path):
    ...
    with gzip.open(fasta_path, "rt") as fasta_file, \
        ...
        for line in tqdm(fasta_file, desc="Processing FASTA", unit=" lines"):
            ...
            if line.startswith(">"):
                if current_sequence and current_acc in filtered_acc2taxid_dict: # we can add the keyword here
                    filtered_fasta_file.write(f">{current_acc}\n")
                    filtered_fasta_file.write("\n".join(current_sequence) + "\n")
                current_sequence = []
                current_acc = line[1:].split()[0]
...
def main():
    ...
    parser.add_argument("--prot_acc2taxid", required=False, help="Path to gzipped protein accession number to taxid mapping file from the NCBI; works with --db nr (if not provided, the latest version will be downloaded from the NCBI", default=False)
    ...
    parser.add_argument("--out", required=True, help="Path to output directory where the results are to be stored.", default=".")
    ...
    #taxdb = check_input(args.taxdb, databases["taxdb"])
    ...
    db_fasta = check_db_path(args.db, args.db_path, args.out)

    if db_name == "nt":
        nucl_gb_acc2taxid = check_input(args.taxdb, databases["nucl_gb_acc2taxid"], args.out)
        nucl_wgs_acc2taxid = check_input(args.taxdb, databases["nucl_wgs_acc2taxid"], args.out)
        accession2taxid = nucl_gb_acc2taxid #os.path.join(args.out, "nt_merged.accession2taxid.gz")
        #merge_gz_files(nucl_gb_acc2taxid, nucl_wgs_acc2taxid, accession2taxid)
        #print("Here is ok4")

    elif db_name == "nr":
        accession2taxid = check_input(args.prot_acc2taxid, databases["prot_acc2taxid"], args.out)
